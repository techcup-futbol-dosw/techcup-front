...
function validateFormLogic(
    ...
    if (profileData.disponible === null) {
        newErrors.disponible = "Indica tu disponibilidad para continuar";
    }
...
export function SportsProfile() {
    ...
    const handleReset = () => {
        setProfileData({ posicion: "", dorsal: "", foto: null, disponible: null });
    ...
    const disponibleButtonStyle = getDisponibleButtonStyle(true, profileData.disponible === true);
    const noDisponibleButtonStyle = getDisponibleButtonStyle(false, profileData.disponible === false);
                            ...
                            <motion.div layout>
                                <fieldset style={{ border: "none", padding: 0, margin: 0 }}>
                                    ...
                                    <div style={{ display: "flex", gap: 12 }}>
                                        <button
                                            type="button"
                                            onClick={() => { setProfileData((prev) => ({ ...prev, disponible: true })); setErrors((prev) => { const c = { ...prev }; delete c.disponible; return c; }); }}
                                            style={disponibleButtonStyle}
                                            aria-pressed={profileData.disponible === true}
                                        >
                                            ✓ Disponible
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => { setProfileData((prev) => ({ ...prev, disponible: false })); setErrors((prev) => { const c = { ...prev }; delete c.disponible; return c; }); }}
                                            style={noDisponibleButtonStyle}
                                            aria-pressed={profileData.disponible === false}
                                        >
                                            ✕ No disponible
                                        </button>
                                    </div>
                                ...
                                {errors.disponible && (
                                    <div role="alert" style={{ color: P.primary, marginTop: 8, display: "flex", alignItems: "center", gap: 6, fontSize: "0.85rem" }}>
                                        <AlertCircle size={14} aria-hidden /> {errors.disponible}
                                    </div>
                                )}
